package// Generated by Yoitsu. DO NOT EDIT!
excels

import (
	"github.com/Fesaa/enka-network-api-go/hash"
	"net/http"
	"io"
	"encoding/json"
)

type OfferingUIPageConfig struct {
	CostTitle     hash.Hash `json:"CostTitle"`
	ID            float64   `json:"ID"`
	LevelTitle    hash.Hash `json:"LevelTitle"`
	LongTailDesc  hash.Hash `json:"LongTailDesc"`
	LongTailTitle hash.Hash `json:"LongTailTitle"`
	MaxTip        hash.Hash `json:"MaxTip"`
	Name          hash.Hash `json:"Name"`
	SubmitBtnName hash.Hash `json:"SubmitBtnName"`
}
type OfferingUIPageConfigAccessor struct {
	_data   []OfferingUIPageConfig
	_dataID map[float64]OfferingUIPageConfig
}

// LoadData retrieves the data. Must be called before OfferingUIPageConfig.GroupData
func (a *OfferingUIPageConfigAccessor) LoadData() error {
	res, err := http.Get("https://gitlab.com/Dimbreath/turnbasedgamedata/-/raw/main/ExcelOutput/OfferingUIPageConfig.json")
	if err != nil {
		return err
	}
	defer res.Body.Close()
	data, err := io.ReadAll(res.Body)
	if err != nil {
		return err
	}
	return json.Unmarshal(data, &a._data)
}

// Raw returns the raw data.
func (a *OfferingUIPageConfigAccessor) Raw() ([]OfferingUIPageConfig, error) {
	if a._data == nil {
		err := a.LoadData()
		if err != nil {
			return []OfferingUIPageConfig{}, err
		}
		a.GroupData()
	}
	return a._data, nil
}

// GroupData groups the data by their unique ids.
// Can be called manually in conjunction with OfferingUIPageConfigAccessor.LoadData to preload everything
func (a *OfferingUIPageConfigAccessor) GroupData() {
	for _, d := range a._data {
		a._dataID[d.ID] = d
	}
}

// ByID returns the OfferingUIPageConfig uniquely identified by ID
//
// Error is only non-nil if the source errors out
func (a *OfferingUIPageConfigAccessor) ByID(identifier float64) (OfferingUIPageConfig, error) {
	if a._dataID == nil {
		err := a.LoadData()
		if err != nil {
			return OfferingUIPageConfig{}, err
		}
		a.GroupData()
	}
	return a._dataID[identifier], nil
}
