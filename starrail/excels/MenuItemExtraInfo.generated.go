package// Generated by Yoitsu. DO NOT EDIT!
excels

import (
	"encoding/json"
	"net/http"
	"io"
)

type MenuItemExtraInfo struct {
	Condition      string        `json:"Condition"`
	ExtraInfoParam string        `json:"ExtraInfoParam"`
	ExtraInfoType  []json.Number `json:"ExtraInfoType"`
	ID             json.Number   `json:"ID"`
}
type MenuItemExtraInfoAccessor struct {
	_data          []MenuItemExtraInfo
	_dataCondition map[string]MenuItemExtraInfo
	_dataID        map[json.Number]MenuItemExtraInfo
}

// LoadData retrieves the data. Must be called before MenuItemExtraInfo.GroupData
func (a *MenuItemExtraInfoAccessor) LoadData() error {
	res, err := http.Get("https://gitlab.com/Dimbreath/turnbasedgamedata/-/raw/main/ExcelOutput/MenuItemExtraInfo.json")
	if err != nil {
		return err
	}
	defer res.Body.Close()
	data, err := io.ReadAll(res.Body)
	if err != nil {
		return err
	}
	return json.Unmarshal(data, &a._data)
}

// Raw returns the raw data.
func (a *MenuItemExtraInfoAccessor) Raw() ([]MenuItemExtraInfo, error) {
	if a._data == nil {
		err := a.LoadData()
		if err != nil {
			return []MenuItemExtraInfo{}, err
		}
	}
	return a._data, nil
}

// GroupData groups the data by their unique ids.
// Can be called manually in conjunction with MenuItemExtraInfoAccessor.LoadData to preload everything
func (a *MenuItemExtraInfoAccessor) GroupData() {
	a._dataCondition = map[string]MenuItemExtraInfo{}
	a._dataID = map[json.Number]MenuItemExtraInfo{}
	for _, d := range a._data {
		a._dataCondition[d.Condition] = d
		a._dataID[d.ID] = d
	}
}

// ByCondition returns the MenuItemExtraInfo uniquely identified by Condition
//
// Error is only non-nil if the source errors out
func (a *MenuItemExtraInfoAccessor) ByCondition(identifier string) (MenuItemExtraInfo, error) {
	if a._dataCondition == nil {
		if a._data == nil {
			err := a.LoadData()
			if err != nil {
				return MenuItemExtraInfo{}, err
			}
		}
		a.GroupData()
	}
	return a._dataCondition[identifier], nil
}

// ByID returns the MenuItemExtraInfo uniquely identified by ID
//
// Error is only non-nil if the source errors out
func (a *MenuItemExtraInfoAccessor) ByID(identifier json.Number) (MenuItemExtraInfo, error) {
	if a._dataID == nil {
		if a._data == nil {
			err := a.LoadData()
			if err != nil {
				return MenuItemExtraInfo{}, err
			}
		}
		a.GroupData()
	}
	return a._dataID[identifier], nil
}
