package// Generated by Yoitsu. DO NOT EDIT!
excels

import (
	"encoding/json"
	"net/http"
	"io"
)

type SwordTrainingUnlock struct {
	FinishWayID json.Number            `json:"FinishWayID"`
	UnlockDesc  map[string]json.Number `json:"UnlockDesc"`
	UnlockID    json.Number            `json:"UnlockID"`
}
type SwordTrainingUnlockAccessor struct {
	_data            []SwordTrainingUnlock
	_dataFinishWayID map[json.Number]SwordTrainingUnlock
	_dataUnlockID    map[json.Number]SwordTrainingUnlock
}

// LoadData retrieves the data. Must be called before SwordTrainingUnlock.GroupData
func (a *SwordTrainingUnlockAccessor) LoadData() error {
	res, err := http.Get("https://gitlab.com/Dimbreath/turnbasedgamedata/-/raw/main/ExcelOutput/SwordTrainingUnlock.json")
	if err != nil {
		return err
	}
	defer res.Body.Close()
	data, err := io.ReadAll(res.Body)
	if err != nil {
		return err
	}
	return json.Unmarshal(data, &a._data)
}

// Raw returns the raw data.
func (a *SwordTrainingUnlockAccessor) Raw() ([]SwordTrainingUnlock, error) {
	if a._data == nil {
		err := a.LoadData()
		if err != nil {
			return []SwordTrainingUnlock{}, err
		}
	}
	return a._data, nil
}

// GroupData groups the data by their unique ids.
// Can be called manually in conjunction with SwordTrainingUnlockAccessor.LoadData to preload everything
func (a *SwordTrainingUnlockAccessor) GroupData() {
	a._dataFinishWayID = map[json.Number]SwordTrainingUnlock{}
	a._dataUnlockID = map[json.Number]SwordTrainingUnlock{}
	for _, d := range a._data {
		a._dataFinishWayID[d.FinishWayID] = d
		a._dataUnlockID[d.UnlockID] = d
	}
}

// ByFinishWayID returns the SwordTrainingUnlock uniquely identified by FinishWayID
//
// Error is only non-nil if the source errors out
func (a *SwordTrainingUnlockAccessor) ByFinishWayID(identifier json.Number) (SwordTrainingUnlock, error) {
	if a._dataFinishWayID == nil {
		if a._data == nil {
			err := a.LoadData()
			if err != nil {
				return SwordTrainingUnlock{}, err
			}
		}
		a.GroupData()
	}
	return a._dataFinishWayID[identifier], nil
}

// ByUnlockID returns the SwordTrainingUnlock uniquely identified by UnlockID
//
// Error is only non-nil if the source errors out
func (a *SwordTrainingUnlockAccessor) ByUnlockID(identifier json.Number) (SwordTrainingUnlock, error) {
	if a._dataUnlockID == nil {
		if a._data == nil {
			err := a.LoadData()
			if err != nil {
				return SwordTrainingUnlock{}, err
			}
		}
		a.GroupData()
	}
	return a._dataUnlockID[identifier], nil
}
